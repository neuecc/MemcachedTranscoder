...
namespace MemcachedTranscoder.MessagePack.Tests
{
    [Serializable]
    [DataContract]
    ...
    public class TranscoderTest
    {
        ...
        {
            var readCache = (ConcurrentDictionary<string, Type>)(typeof(MessagePackTranscoder).GetField("readCache", BindingFlags.Static | BindingFlags.GetField | BindingFlags.NonPublic).GetValue(null));
            return readCache;
        }

        public ConcurrentDictionary<Type, string> GetWriteCache()
        {
            var writeCache = (ConcurrentDictionary<Type, string>)(typeof(MessagePackTranscoder).GetField("writeCache", BindingFlags.Static | BindingFlags.GetField | BindingFlags.NonPublic).GetValue(null));
            return writeCache;
        }
        ...
        public void RunTranscodeTest<T>(T value, Action<T, T> checker)
        {
            ...
            var transcoder = (ITranscoder)new MessagePackTranscoder();
